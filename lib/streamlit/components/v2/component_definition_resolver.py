...
def build_definition_with_validation(
    *,
    ...
    caller_dir : str
        Absolute directory of the Python file that invoked the API. Used to
        resolve relative file paths provided via ``css`` or ``js``.
    html : str | None
    ...
    - Globs are supported only within ``asset_dir`` and must resolve to exactly
      one file.
    - Relative paths are resolved against ``caller_dir`` and must remain within
